...
class PerformanceTelemetryServiceTest {
	...
	void recordsDurationRateVersionConfigAndPhases() {
		...
		PerformanceTelemetryService service = new PerformanceTelemetryService(executionRepository,
				executionPhaseRepository, "9.9.9.9", Clock.system(ZoneId.of("UTC")));

		service.recordMetrics(7L, new ConfigSnapshot(4, 200, 2, 3), Map.of(ExecutionPhaseType.CACHE_CHECK,
				new PhaseSnapshot(50, 0), ExecutionPhaseType.EXTRACTION, new PhaseSnapshot(8000, 100)));

		Assertions.assertThat(execution.getMetrics().getDurationMillis()).isEqualTo(10_000);
		Assertions.assertThat(execution.getMetrics().getFilesPerSecond()).isEqualTo(10.0);
		Assertions.assertThat(execution.getApplicationVersion()).isEqualTo("9.9.9.9");
		Assertions.assertThat(execution.getMetrics().getWorkers()).isEqualTo(4);
		Assertions.assertThat(execution.getMetrics().getChunkSize()).isEqualTo(200);
		Assertions.assertThat(execution.getMetrics().getFfmpegPhotoHashLimit()).isEqualTo(2);
		Assertions.assertThat(execution.getMetrics().getFfprobeVideoLimit()).isEqualTo(3);
	...
	void recordsPhotoHashCountersWhenProvided() {
		...
		PerformanceTelemetryService service = new PerformanceTelemetryService(executionRepository,
				executionPhaseRepository, "v", Clock.system(ZoneId.of("UTC")));

		service.recordMetrics(8L, null, Map.of(), new PhotoHashCounters(17683, 84, 52));

		Assertions.assertThat(execution.getMetrics().getPhotoHashJvmDecodable()).isEqualTo(17683L);
		Assertions.assertThat(execution.getMetrics().getPhotoHashFfmpegOnly()).isEqualTo(84L);
		Assertions.assertThat(execution.getMetrics().getPhotoHashFailures()).isEqualTo(52L);
	}
	...
	void usesFilesMovedForOrganizationRateWhenNoFilesFound() {
		...
		service.recordMetrics(11L, null, Map.of());

		Assertions.assertThat(execution.getMetrics().getFilesPerSecond()).isEqualTo(20.0);
	}
	...
	void zeroDurationYieldsZeroFilesPerSecondInsteadOfDividingByZero() {
		...
		service.recordMetrics(12L, null, Map.of());

		Assertions.assertThat(execution.getMetrics().getDurationMillis()).isZero();
		Assertions.assertThat(execution.getMetrics().getFilesPerSecond()).isZero();
	}
	...
	void computesTrueElapsedAcrossADaylightSavingSpringForward() {
		...
		PerformanceTelemetryService service = new PerformanceTelemetryService(executionRepository,
				executionPhaseRepository, "v", Clock.system(ZoneId.of("America/New_York")));

		service.recordMetrics(21L, null, Map.of());
		...
		Assertions.assertThat(execution.getMetrics().getDurationMillis()).isEqualTo(1_800_000L);
	}
	...
	void ignoresAMissingExecution() {
		...
		service.recordMetrics(9L, null, Map.of());
